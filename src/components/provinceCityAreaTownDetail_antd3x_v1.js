...
import {
    ...
    city as getCityList,
    area as getAreaList,
    getTownStreetByParentId as getTownList
...
export default forwardRef( (props , ref) => {
  ...
  const { province , city , area , town , detail } = value;
  const { useProvince , useCity , useArea , useTown , useDetail } = config;
  ...
  const apiTown = params =>{
    setTownLoading(true);
    useTown && getTownList(params).then( res => {
      const {code , data} = res;
      if(code === 0){
        setTownList(data);
      }else{
      ...
      }
      {
        ...
        <Item>
          ...
          onChange = { (id)=> handleSelectChange(id , "town") }>
            { Array.isArray(townList) && townList.map( item => <Option value={ item.townId } key={item.townId}>{item.townId}</Option>) } 
          </Select>
        </Item>
      }
      { 
